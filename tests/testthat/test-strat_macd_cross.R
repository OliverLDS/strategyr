...
test_that("strat_macd_contrarian_tgt_pos is inverse MACD cross", {
  DT <- data.table(
    macd_12_26 = c(-1, 0.5, -0.2),
    macd_signal_12_26_9 = c(0, 0, 0)
  )
...
test_that("strat_macd_contrarian_tgt_pos preserves debug metadata", {
  DT <- data.table(
    macd_12_26 = c(-1, 0.5),
    macd_signal_12_26_9 = c(0, 0)
  )

  res <- strat_macd_contrarian_tgt_pos(
    DT,
    fast = 12L,
    slow = 26L,
    signal = 9L,
  ...
  expect_equal(res$tgt_pos, c(1, -1))
  expect_equal(res$feature_cols, c("macd_12_26", "macd_signal_12_26_9"))
})

test_that("strat_macd_contrarian_action_plan uses latest inverse target", {
  DT <- data.table(
    macd_12_26 = c(-1, 0.5),
    macd_signal_12_26_9 = c(0, 0)
  )
  ...
  plan <- strat_macd_contrarian_action_plan(
    DT,
    state,
    fast = 12L,
    slow = 26L,
  ...
  expect_equal(plan$n, 1L)
  expect_equal(plan$actions[[1]]$dir, -1L)
  expect_equal(plan$actions[[1]]$strat, 305L)
})
