...
test_that("strat_gamma_scalp_support_tgt_pos activates when realized exceeds implied", {
  DT <- data.table(time_to_expiry = c(30 / 252, 30 / 252, 30 / 252), gamma_scalp_edge_20 = c(-0.05, 0.01, 0.02))
...
test_that("strat_gamma_scalp_support_action_plan can attach a hedge plan", {
  DT <- data.table(time_to_expiry = c(30 / 252, 30 / 252), gamma_scalp_edge_20 = c(0.01, 0.02))
