...
#' @noRd
.validate_option_inputs <- function(S, K, time_to_expiry, r, sigma, q = 0) {
  stopifnot(is.numeric(S), length(S) == 1, is.finite(S), S > 0)
  stopifnot(is.numeric(K), length(K) == 1, is.finite(K), K > 0)
  stopifnot(is.numeric(time_to_expiry), length(time_to_expiry) == 1, is.finite(time_to_expiry), time_to_expiry > 0)
  ...
  list(
    S = as.numeric(S),
    K = as.numeric(K),
    time_to_expiry = as.numeric(time_to_expiry),
    r = as.numeric(r),
...
#' @noRd
.bs_d1 <- function(S, K, time_to_expiry, r, sigma, q = 0) {
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  (log(x$S / x$K) + (x$r - x$q + 0.5 * x$sigma^2) * x$time_to_expiry) / (x$sigma * sqrt(x$time_to_expiry))
}
...
#' @noRd
.bs_d2 <- function(S, K, time_to_expiry, r, sigma, q = 0) {
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  .bs_d1(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q) - x$sigma * sqrt(x$time_to_expiry)
}
...
#' @noRd
.calc_option_price_bs <- function(S, K, time_to_expiry, r, sigma, type, q = 0) {
  type <- .validate_option_type(type)
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  d1 <- .bs_d1(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q)
  d2 <- d1 - x$sigma * sqrt(x$time_to_expiry)
  df_q <- exp(-x$q * x$time_to_expiry)
  df_r <- exp(-x$r * x$time_to_expiry)
...
#' @export
calc_option_delta <- function(S, K, time_to_expiry, r, sigma, type = c("call", "put"), q = 0) {
  type <- .validate_option_type(type)
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  d1 <- .bs_d1(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q)
  df_q <- exp(-x$q * x$time_to_expiry)
...
#' @export
calc_option_gamma <- function(S, K, time_to_expiry, r, sigma, q = 0) {
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  d1 <- .bs_d1(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q)
  exp(-x$q * x$time_to_expiry) * stats::dnorm(d1) / (x$S * x$sigma * sqrt(x$time_to_expiry))
}
...
#' @export
calc_option_vega <- function(S, K, time_to_expiry, r, sigma, q = 0) {
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  d1 <- .bs_d1(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q)
  x$S * exp(-x$q * x$time_to_expiry) * stats::dnorm(d1) * sqrt(x$time_to_expiry)
}
...
#' @export
calc_option_theta <- function(S, K, time_to_expiry, r, sigma, type = c("call", "put"), q = 0, scale = c("annual", "daily")) {
  type <- .validate_option_type(type)
  ...
  common <- -(x$S * df_q * stats::dnorm(d1) * x$sigma) / (2 * sqrt(x$time_to_expiry))
  theta <- if (type == "call") {
...
#' @export
calc_option_rho <- function(S, K, time_to_expiry, r, sigma, type = c("call", "put"), q = 0) {
  type <- .validate_option_type(type)
  x <- .validate_option_inputs(S, K, time_to_expiry, r, sigma, q = q)
  d2 <- .bs_d2(x$S, x$K, x$time_to_expiry, x$r, x$sigma, q = x$q)
  df_r <- exp(-x$r * x$time_to_expiry)

  if (type == "call") {
    x$K * x$time_to_expiry * df_r * stats::pnorm(d2)
  } else {
    -x$K * x$time_to_expiry * df_r * stats::pnorm(-d2)
  }
}
...
#'   `theta`, and `rho`.
#' @export
calc_option_greeks <- function(S, K, time_to_expiry, r, sigma, type = c("call", "put"), q = 0, theta_scale = c("annual", "daily")) {
  type <- .validate_option_type(type)
  theta_scale <- match.arg(theta_scale)

  data.table::data.table(
    price = .calc_option_price_bs(S, K, time_to_expiry, r, sigma, type = type, q = q),
    delta = calc_option_delta(S, K, time_to_expiry, r, sigma, type = type, q = q),
    gamma = calc_option_gamma(S, K, time_to_expiry, r, sigma, q = q),
    vega = calc_option_vega(S, K, time_to_expiry, r, sigma, q = q),
    theta = calc_option_theta(S, K, time_to_expiry, r, sigma, type = type, q = q, scale = theta_scale),
    rho = calc_option_rho(S, K, time_to_expiry, r, sigma, type = type, q = q)
  )
}
...
#' @export
calc_option_iv <- function(price, S, K, time_to_expiry, r, type = c("call", "put"), q = 0, interval = c(1e-6, 5)) {
  type <- .validate_option_type(type)
  ...
  f <- function(sigma) {
    .calc_option_price_bs(S, K, time_to_expiry, r, sigma, type = type, q = q) - price
  }
