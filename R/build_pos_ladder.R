...
.build_pos_ladder <- function(idx, w = NULL, k = 3L, dir = c('both', 'long', 'short'), mode = c('both', 'continuous', 'reversal'), breakout = TRUE) {
...
#' @export
build_pos_ladder <- function(DT, idx_col_name = 'ind_dca_ladder_360', pos_col_name = 'pos_dca_ladder_360', w = NULL, k = 3L, dir = c('both', 'long', 'short'), mode = c('both', 'continuous', 'reversal'), breakout = TRUE) {
  dir <- match.arg(dir)
  mode <- match.arg(mode)
  invisible(DT[, (pos_col_name) := .build_pos_ladder(DT[[idx_col_name]], w = w, k = k, dir = dir, mode = mode, breakout = breakout)])
}
