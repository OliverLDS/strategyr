.roll_yield_cross_sectional_weights <- function(signal, long_n = 1L, short_n = 0L, gross_exposure = 1.0) {
...
#'
#' Generates per-row target weights by ranking a roll-yield signal across
#' assets within each date. Top-ranked assets receive positive weights and
#' optionally bottom-ranked assets receive negative weights.
#'
...
#' @param signal_col Roll-yield signal column used for ranking.
...
#' @export
strat_roll_yield_cross_sectional_tgt_pos <- function(DT, date_col = "date", asset_col = "asset", signal_col = "roll_yield", long_n = 1L, short_n = 0L, gross_exposure = 1.0, debug = FALSE) {
  .validate_market_dt(DT, c(date_col, asset_col, signal_col))

  tgt_pos <- DT[, .roll_yield_cross_sectional_weights(
...
#'
#' Applies the latest cross-sectional roll-yield target weights to a portfolio
#' state and converts them into a portfolio-adjustment table plus order intents.
#'
#' @inheritParams strat_roll_yield_cross_sectional_tgt_pos
...
#'   weights.
#' @export
strat_roll_yield_cross_sectional_action_plan <- function(DT, portfolio_state, equity, date_col = "date", asset_col = "asset", signal_col = "roll_yield", long_n = 1L, short_n = 0L, gross_exposure = 1.0, strat_id = 509L, pricing_method = "market", debug = FALSE) {
  .validate_market_dt(DT, c(date_col, asset_col, signal_col))
  stopifnot(data.table::is.data.table(portfolio_state))

  work_dt <- data.table::copy(DT)
  tgt_pos <- strat_roll_yield_cross_sectional_tgt_pos(
    work_dt,
    date_col = date_col,
    asset_col = asset_col,
