...
param_grid <- data.table::CJ(
  trend_n = c(20L, 50L, 100L),
  rsi_n = c(10L, 14L, 21L),
  pullback_long = c(40, 55),
  pullback_short = c(45, 60),
  exit_rsi = 50,
  target_size = 0.95
)
...
# Trend pullback is a price-path rule. For this example, skip yield indices and
...
trend_pullback_mining_res <- strategyr::mine_strategy_asset_years(
  market_data_list = asset_market_data,
  strategy_fun = strategyr::strat_trend_pullback_tgt_pos,
...
print(trend_pullback_mining_res$seed_params[, .(
  seed_asset,
  rank,
  trend_n,
  rsi_n,
...
print(trend_pullback_mining_res$candidate_params)

asset_year_res <- trend_pullback_mining_res$asset_year_results
