...
strat_ema_cross_scaled_tgt_pos <- function(
  DT,
  fast,
  slow,
  low_atr_threshold,
  ...
  strategyr::strat_ema_cross_tgt_pos(
    DT,
    fast = fast,
    slow = slow,
...
param_grid <- data.table::CJ(
  fast = c(10L, 20L, 30L),
  slow = c(50L, 100L),
  low_atr_threshold = c(25L, 50L),
  freshness_floor = c(60L, 250L),
  tp_ratio = c(0.10, 0.20),
  sl_ratio = c(0.05, 0.10),
  target_size = 0.95
)[fast < slow]

registry_dt <- data.table::as.data.table(jsonlite::fromJSON(registry_path))

# EMA cross is a price-path rule. For this example, skip yield indices and
...
ema_cross_mining_res <- strategyr::mine_strategy_asset_years(
  market_data_list = asset_market_data,
  strategy_fun = strat_ema_cross_scaled_tgt_pos,
...
print(ema_cross_mining_res$seed_params[, .(
  seed_asset,
  rank,
  fast,
  slow,
...
print(ema_cross_mining_res$candidate_params)

asset_year_res <- ema_cross_mining_res$asset_year_results
