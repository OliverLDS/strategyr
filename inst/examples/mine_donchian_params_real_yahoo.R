...
if (is.null(market_dt)) {
  stop("Need at least 260 rows of local OHLC data for Donchian parameter mining: ", ticker)
}
...
donchian_mining_res <- strategyr::mine_strategy_params(
  DT = market_dt,
...
best_donchian_params <- donchian_mining_res[1L]

print(best_donchian_params)
print(donchian_mining_res[seq_len(min(.N, 10L)), .(
  rank,
  n,
  target_size,
  sortino,
...
best_strategy_params <- as.list(
  best_donchian_params[, .(n, target_size)]
)

donchian_asset_res <- strategyr::mine_strategy_assets(
...
print(donchian_asset_res[seq_len(min(.N, 20L)), .(
  rank,
  asset,
  sortino,
  total_return,
