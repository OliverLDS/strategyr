...
load_local_yahoo <- function(symbol, min_rows = 520L) {
  out <- tryCatch(
...
param_grid <- data.table::CJ(
  rsi_n = c(10L, 14L, 21L),
  threshold_n = c(126L, 252L),
  lower_q = c(0.05, 0.10, 0.20),
  upper_q = c(0.80, 0.90, 0.95),
  exit_level = c(0.45, 0.50, 0.55),
  target_size = 0.95
)[lower_q < exit_level & exit_level < upper_q]
...
mining_res <- strategyr::mine_strategy_asset_years(
  ...
  strategy_fun = strategyr::strat_rsi_dynamic_threshold_revert_tgt_pos,
...
print(mining_res$seed_params[, .(seed_asset, rank, rsi_n, threshold_n, lower_q, upper_q, exit_level, sortino, total_return, max_drawdown)])
print(mining_res$candidate_params)
print(mining_res$asset_year_results[seq_len(min(.N, 20L)), .(
  rank, asset, year, param_id, rsi_n, threshold_n, lower_q, upper_q,
  exit_level, sortino, total_return, buy_hold_total_return,
  excess_total_return, warmup_n_obs, warmup_insufficient, n_obs
)])
