...
if (length(asset_market_data) < 2L) {
  stop("Need at least two locally cached Yahoo assets for ratio-reversion mining.")
}

ratio_revert_mining_res <- strategyr::mine_strategy_asset_years(
  market_data_list = asset_market_data,
  strategy_fun = strategyr::strat_ratio_revert_tgt_pos,
...
print(ratio_revert_mining_res$seed_params[, .(
  seed_asset,
  rank,
  z_n,
  entry_z,
...
print(ratio_revert_mining_res$candidate_params)

asset_year_res <- ratio_revert_mining_res$asset_year_results
