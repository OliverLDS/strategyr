...
param_grid <- data.table::CJ(
  bb_n = c(15L, 20L, 30L, 40L),
  bb_k = c(1.5, 2, 2.5),
  squeeze_width = c(0.03, 0.05, 0.08),
  target_size = 0.95
)
...
mining_res <- strategyr::mine_strategy_asset_years(
  ...
  strategy_fun = strategyr::strat_bollinger_squeeze_breakout_tgt_pos,
...
print(mining_res$seed_params[, .(seed_asset, rank, bb_n, bb_k, squeeze_width, sortino, total_return, max_drawdown)])
print(mining_res$candidate_params)
print(mining_res$asset_year_results[seq_len(min(.N, 20L)), .(
  rank, asset, year, param_id, bb_n, bb_k, squeeze_width, sortino,
