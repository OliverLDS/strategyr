...
# deeper offline mining job. The gallery settings are cycle = 180 and 7/13.
param_grid <- data.table::CJ(
  cycle_N = c(90L, 180L, 252L),
  span = 3L,
  refined = TRUE,
  min_swing = c(0.03, 0.05),
  center_idx = 9L,
  lower = c(6L, 7L),
  upper = c(12L, 13L),
  target_size = 0.95
...
if (length(asset_market_data) < 2L) {
  stop("Need at least two locally cached Yahoo assets for ladder-breakout mining.")
}

ladder_breakout_mining_res <- strategyr::mine_strategy_asset_years(
  market_data_list = asset_market_data,
  strategy_fun = strategyr::strat_ladder_breakout_tgt_pos,
...
print(ladder_breakout_mining_res$seed_params[, .(
  seed_asset,
  rank,
  cycle_N,
  span,
...
print(ladder_breakout_mining_res$candidate_params)

asset_year_res <- ladder_breakout_mining_res$asset_year_results
